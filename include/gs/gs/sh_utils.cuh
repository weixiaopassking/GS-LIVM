...
inline torch::Tensor Eval_sh(int deg, const torch::Tensor& sh, const torch::Tensor& dirs) {
  ...
  if (deg > 0) {
    ...
    result = result - C1 * y * sh.index({torch::indexing::Ellipsis, 1}) + C1 * z * sh.index({torch::indexing::Ellipsis, 2}) -
             C1 * x * sh.index({torch::indexing::Ellipsis, 3});
    if (deg > 1) {
      auto xx = x * x, yy = y * y, zz = z * z;
      auto xy = x * y, yz = y * z, xz = x * z;
      result = result + C2[0] * xy * sh.index({torch::indexing::Ellipsis, 4}) + C2[1] * yz * sh.index({torch::indexing::Ellipsis, 5}) +
               C2[2] * (2.0 * zz - xx - yy) * sh.index({torch::indexing::Ellipsis, 6}) + C2[3] * xz * sh.index({torch::indexing::Ellipsis, 7}) +
               C2[4] * (xx - yy) * sh.index({torch::indexing::Ellipsis, 8});
      if (deg > 2) {
        result = result + C3[0] * y * (3 * xx - yy) * sh.index({torch::indexing::Ellipsis, 9}) + C3[1] * xy * z * sh.index({torch::indexing::Ellipsis, 10}) +
                 ...
                 C3[5] * z * (xx - yy) * sh.index({torch::indexing::Ellipsis, 14}) + C3[6] * x * (xx - 3 * yy) * sh.index({torch::indexing::Ellipsis, 15});
        if (deg > 3) {
