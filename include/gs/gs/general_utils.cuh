  ...
  float operator()(int64_t step) const {
    ...
    if (lr_delay_steps > 0. && step != 0) {
      delay_rate = lr_delay_mult +
                   (1 - lr_delay_mult) * std::sin(0.5 * M_PI * std::clamp((float)step / lr_delay_steps, 0.f, 1.f));
    } else {
      delay_rate = 1.0;
    }
...
inline torch::Tensor strip_lowerdiag(torch::Tensor L) {
  torch::Tensor uncertainty = torch::zeros({L.size(0), 6}, torch::dtype(torch::kFloat32).device(torch::kCUDA));
...
inline torch::Tensor build_scaling_rotation(torch::Tensor s, torch::Tensor r) {
  torch::Tensor L = torch::zeros({s.size(0), 3, 3}, torch::dtype(torch::kFloat32).device(torch::kCUDA));
