...
enum { cv_OPTFLOW_USE_INITIAL_FLOW = 4, cv_OPTFLOW_LK_GET_MIN_EIGENVALS = 8, cv_OPTFLOW_FARNEBACK_GAUSSIAN = 256 };
...
void calculateOpticalFlow(
    cv::InputArray prevImg_,
    cv::InputArray nextImg_,
    cv::InputArray prevPts_,
    cv::InputOutputArray nextPts_,
    cv::OutputArray status_,
    cv::OutputArray err_,
...
struct opencvLKTrackerInvoker : cv::ParallelLoopBody {
  opencvLKTrackerInvoker(
      const cv::Mat* prevImg_,
      const cv::Mat* prevDeriv_,
      const cv::Mat* nextImg_,
      const cv::Point2f* prevPts_,
      cv::Point2f* nextPts_,
      uchar* status_,
      float* err_,
      cv::Size winSize_,
...
class LKOpticalFlowKernel {
 ...
 public:
  ...
  void calc(
      ...
      cv::InputOutputArray nextPts_,
      cv::OutputArray status_,
      cv::OutputArray err_ = cv::noArray());
  ...
  int trackImage(
      ...
      int opm_method = 3);  // opm_method: [0] openCV parallel_body [1] openCV parallel for [2] Thread pool
};