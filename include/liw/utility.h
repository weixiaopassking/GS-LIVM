...
#define MIN_INI_COUNT 20
#define MIN_INI_TIME 0.2
...
void pointBodyToWorld(
    pcl::PointXYZINormal const* const pi,
    pcl::PointXYZINormal* const po,
    Eigen::Matrix3d& R_world_cur,
    Eigen::Vector3d& t_world_cur,
    Eigen::Matrix3d& R_imu_lidar,
    Eigen::Vector3d& t_imu_lidar);

void RGBpointLidarToIMU(
    pcl::PointXYZINormal const* const pi,
    pcl::PointXYZINormal* const po,
    Eigen::Matrix3d& R_imu_lidar,
    Eigen::Vector3d& t_imu_lidar);
...
bool planeFitting(
    Eigen::Matrix<double, 4, 1>& plane_parameter,
...
enum SizeParameterization { NUM_MATCH_POINTS = 20 };

enum MotionCompensation { IMU = 0, CONSTANT_VELOCITY = 1 };

enum StateInitialization { INIT_IMU = 0, INIT_CONSTANT_VELOCITY = 1 };
...
class numType {
 public:
  ...
  template <typename Derived>
  static Eigen::Matrix<typename Derived::Scalar, 3, 1> rotFromV1toV2(
      const Eigen::MatrixBase<Derived>& v1_in,
      const Eigen::MatrixBase<Derived>& v2_in) {
    typedef typename Derived::Scalar Scalar_t;
    ...
    Eigen::Matrix<Scalar_t, 3, 3> R = Eigen::Matrix<Scalar_t, 3, 3>::Identity() + skew +
                                      skew * skew * (static_cast<Scalar_t>(1.0) - dot) /
  ...
  static Eigen::Matrix<typename Derived::Scalar, 3, 1> rotationToSo3(const Eigen::MatrixBase<Derived>& R_in) {
    ...
    if (theta < static_cast<Scalar_t>(THETA_THRESHOLD))
      return Eigen::Matrix<Scalar_t, 3, 1>(R(2, 1) - R(1, 2), R(0, 2) - R(2, 0), R(1, 0) - R(0, 1)) /
             static_cast<Scalar_t>(2.0);
    else
      return theta * Eigen::Matrix<Scalar_t, 3, 1>(R(2, 1) - R(1, 2), R(0, 2) - R(2, 0), R(1, 0) - R(0, 1)) /
             (static_cast<Scalar_t>(2.0) * sin(theta));
  }
  ...
  static Eigen::Matrix<typename Derived::Scalar, 3, 3> so3ToRotation(const Eigen::MatrixBase<Derived>& so3_in) {
    ...
    if (theta < static_cast<Scalar_t>(THETA_THRESHOLD)) {
      ...
      return Eigen::Matrix<Scalar_t, 3, 3>::Identity() + sin(theta) * u_x +
             (static_cast<Scalar_t>(1) - cos(theta)) * u_x * u_x;
    }
  }
  ...
  static Eigen::Quaternion<typename Derived::Scalar> so3ToQuat(const Eigen::MatrixBase<Derived>& so3_in) {
    ...
    if (theta < static_cast<Scalar_t>(THETA_THRESHOLD)) {
      ...
      Eigen::Quaternion<Scalar_t> q(
          cos(static_cast<Scalar_t>(0.5) * theta),
          u.x() * sin(static_cast<Scalar_t>(0.5) * theta),
          u.y() * sin(static_cast<Scalar_t>(0.5) * theta),
          u.z() * sin(static_cast<Scalar_t>(0.5) * theta));
      q.normalize();
      return q;
    }
  }
  ...
  static Eigen::Matrix<typename Derived::Scalar, 3, 3> JleftSo3(const Eigen::MatrixBase<Derived>& so3_in) {
    ...
    if (theta < static_cast<Scalar_t>(THETA_THRESHOLD)) {
      ...
      Eigen::Matrix<Scalar_t, 3, 1> u = so3_in.normalized();
      return sin(theta) / theta * Eigen::Matrix<Scalar_t, 3, 3>::Identity() +
             (static_cast<Scalar_t>(1) - sin(theta) / theta) * u * u.transpose() +
  ...
  static Eigen::Matrix<typename Derived::Scalar, 3, 3> invJleftSo3(const Eigen::MatrixBase<Derived>& so3_in) {
    ...
    if (theta < static_cast<Scalar_t>(THETA_THRESHOLD)) {
      return cos(theta / static_cast<Scalar_t>(2)) * Eigen::Matrix<Scalar_t, 3, 3>::Identity() +
             static_cast<Scalar_t>(0.125) * so3_in * so3_in.transpose() -
             static_cast<Scalar_t>(0.5) * skewSymmetric(so3_in);
    } else {
      Eigen::Matrix<Scalar_t, 3, 1> u = so3_in.normalized();
      return static_cast<Scalar_t>(0.5) * theta / tan(theta / static_cast<Scalar_t>(2)) *
                 Eigen::Matrix<Scalar_t, 3, 3>::Identity() +
             (static_cast<Scalar_t>(1) - static_cast<Scalar_t>(0.5) * theta / tan(theta / static_cast<Scalar_t>(2))) *
                 u * u.transpose() -
             static_cast<Scalar_t>(0.5) * skewSymmetric(so3_in);
    }
  }
