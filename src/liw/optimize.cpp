...
optimizeSummary lioOptimization::buildPlaneResiduals(
  ...
  auto estimatePointNeighborhood =
      [&](std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& vector_neighbors, Eigen::Vector3d& location, double& planarity_weight) {
  ...
  for (int k = 0; k < num_keypoints; k++) {
    ...
    auto vector_neighbors = searchNeighbors(
        ...
        keypoint.point,
        nb_voxels_visited,
        cur_icp_options.size_voxel_map,
        cur_icp_options.max_number_neighbors,
        kThresholdCapacity,
    ...
    weight = lambda_weight * weight + lambda_neighborhood * std::exp(-(vector_neighbors[0] - keypoint.point).norm() / (kMaxPointToPlane * kMinNumNeighbors));
    ...
    if (plane_temp.distance < cur_icp_options.max_dist_to_plane_icp) {
      ...
      plane_temp.jacobians.block<1, 3>(0, 3) =
          -plane_temp.norm_vector.transpose() * end_quat.toRotationMatrix() * numType::skewSymmetric(plane_temp.raw_point) * weight;
      plane_residuals.push_back(plane_temp);

      loss_sum += plane_temp.distance * plane_temp.distance;
    }

    if (num_residuals >= cur_icp_options.max_num_residuals)
      break;
  }
...
optimizeSummary
lioOptimization::updateIEKF(const icpOptions& cur_icp_options, voxelHashMap& voxel_map_temp, std::vector<point3D>& keypoints, cloudFrame* p_frame) {
  int max_num_iter = p_frame->frame_id < cur_icp_options.init_num_frames ? std::max(15, cur_icp_options.num_iters_icp) : cur_icp_options.num_iters_icp;
  ...
  for (int i = -1; i < max_num_iter; i++) {
    ...
    if (fabs(1.0 - dot) < 1e-6)
      ...
      R_dg = Eigen::Matrix3d::Identity() + skew + skew * skew * (1.0 - dot) / (cross(0) * cross(0) + cross(1) * cross(1) + cross(2) * cross(2));
    }
    ...
    if (p_frame->frame_id > 1 && (d_x.head<3>()).norm() < cur_icp_options.threshold_translation_norm &&
...
Neighborhood lioOptimization::computeNeighborhoodDistribution(const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& points) {
  ...
  for (auto& point : points) {
    for (int k = 0; k < 3; ++k)
      for (int l = k; l < 3; ++l)
        covariance_Matrix(k, l) += (point(k) - barycenter(k)) * (point(l) - barycenter(l));
  }
...
std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>> lioOptimization::searchNeighbors(
    voxelHashMap& map,
