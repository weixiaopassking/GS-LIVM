...
bool planeFitting(
  ...
  for (int i = 0; i < NUM_MATCH_POINTS; i++) {
    if (fabs(
            plane_parameter(0, 0) * point[i].x + plane_parameter(1, 0) * point[i].y +
            plane_parameter(2, 0) * point[i].z + plane_parameter(3, 0)) > threshold)
      return false;
  }
...
float calculateDist2(pcl::PointXYZINormal point1, pcl::PointXYZINormal point2) {
  float d = (point1.x - point2.x) * (point1.x - point2.x) + (point1.y - point2.y) * (point1.y - point2.y) +
...
void distortFrameByConstant(
  ...
  while (iter != points.end()) {
    ...
    if (fabs(time_point - time_frame_begin) < 1e-6)
      time_point = time_frame_begin + 1e-6;
    if (fabs(time_point - time_frame_end) < 1e-6)
      time_point = time_frame_end - 1e-6;
  ...
  for (int n = 0; n + 1 < imu_states.size(); n++) {
    ...
    while (iter != points.end()) {
      ...
      if (time_point > time_imu_begin - 1e-6 && time_point < time_imu_end + 1e-6) {
        if (fabs(time_point - time_imu_begin) < 1e-6)
          time_point = time_imu_begin + 1e-6;
        if (fabs(time_point - time_imu_end) < 1e-6)
          time_point = time_imu_end - 1e-6;
        ...
        (*iter).imu_point =
            quat_point.toRotationMatrix() * (R_imu_lidar * (*iter).raw_point + t_imu_lidar) + trans_point;
        // distortion method 1
...
void transformAllImuPoint(
  ...
  while (iter != points.end()) {
    (*iter).raw_point =
        R_imu_lidar.transpose() * (quat_end_inv.toRotationMatrix() * (*iter).imu_point + trans_end_inv) -
