...
// some extra large 3dgs will not be renderes
__forceinline__ __device__ bool scale_abnormal(const glm::vec3 scale, float mod) {
  float s_x = mod * scale.x;
  float s_y = mod * scale.y;
  float s_z = mod * scale.z;

  return s_x > 0.3f || s_y > 0.3f || s_z > 0.3f;
}
...
__global__ void preprocessCUDA(
  ...
  if (scale_abnormal(scales[idx], scale_modifier)) {
    return;
  }
